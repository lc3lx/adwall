...
import { type Ad, type Coupon, type User, type Category } from "@/types/types"

let adsStore: Ad[] = [...initialAds]
let couponsStore: Coupon[] = [{ code: "WELCOME10", percent: 10, active: true }]
let usersStore: User[] = [
  { id: "admin1", email: "admin@adwell.io", role: "admin", name: "Admin" },
]
...
export function addAd(ad: Ad) {
  adsStore = [ad, ...adsStore]
  if (ad.ownerEmail) {
    const exists = usersStore.find((u) => u.email.toLowerCase() === ad.ownerEmail!.toLowerCase())
    if (!exists) {
      usersStore = [{ id: uid(), email: ad.ownerEmail, role: "advertiser", phone: ad.phone, name: ad.companyName }, ...usersStore]
    } else {
